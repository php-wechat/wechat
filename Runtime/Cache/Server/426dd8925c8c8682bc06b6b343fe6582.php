...
<?php if(($info["id"]) > "0"): ?><form action="{viicms::U('User/edit')}" method="post" name="form" id="myform">
			<input type="hidden" name="id" value="{viicms:$info.id}">
		<?php else: ?>
			<form action="{viicms::U('User/add')}" method="post" name="form" id="myform"><?php endif; ?>
				...
				<tr>
					...
					<td colspan="3" class="lt">
						<select name="role" style="width:136px">
							<?php if(is_array($role)): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="{viicms:$vo.id}" <?php if(($vo["id"]) == $info["role"]): ?>selected=""<?php endif; ?> >{viicms:$vo.name}</option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</td>
				</tr>
				<tr>
