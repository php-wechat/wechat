...
<?php if(($info["id"]) > "0"): ?><script src="/Public/common/date/WdatePicker.js"></script>

			<form action="{viicms::U('Users/edit')}" method="post" name="form" id="myform">
			<input type="hidden" name="id" value="{viicms:$info.id}">
		<?php else: ?>
			<form action="{viicms::U('Users/add')}" method="post" name="form" id="myform"><?php endif; ?>
			<table width="100%" border="0" cellspacing="0" cellpadding="0" id="addn">
				...
				<tr>
					...
					<td colspan="3" class="lt">
						<input type="text" name="username" class="ipt" size="45" value="{viicms:$info.username}" <?php if(($info["username"]) == "admin"): ?>readonly="readonly"<?php endif; ?>>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="3" class="lt">
						<select name="gid" style="width:136px">
							<?php if(is_array($role)): $i = 0; $__LIST__ = $role;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="{viicms:$vo.id}" <?php if(($vo["id"]) == $info["gid"]): ?>selected=""<?php endif; ?> >{viicms:$vo.name}</option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td colspan="3" class="lt">
						<input type="text" name="money" class="ipt" size="45" value="{viicms:$info.money}">
					</td>
				</tr>
				<tr>
					<td height="48" align="right"><strong>到期时间：</strong></td>
					<td colspan="3" class="lt">
						<input type="text" name="viptime" onClick="WdatePicker()" class="ipt" size="45" value="{viicms:$info.viptime}">
					</td>
				</tr>
				<tr>
					<td height="48" align="right"><strong>备注说明：</strong></td>
					<td colspan="3" class="lt">
						<input type="text" name="remark" class="ipt" size="45" value="{viicms:$info.remark}"/>
					</td>
				</tr>
	<tr>
		<td colspan="4">
