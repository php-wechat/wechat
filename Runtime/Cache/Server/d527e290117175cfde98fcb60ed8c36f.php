...
<div id="artlist">
    <div class="mod kjnav">
        <a href="<?php echo U('UserGroup/add');?>">创建用户</a>
        <a href="<?php echo U('UserGroup/index');?>">用户列表</a>
    </div>
</div>
<div class="cr"></div>
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="alist">
	  <tr>
	    ...
	    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td >{viicms:$vo['price']}</td>
				<td >{viicms:$vo.diynum}</td>
				<td align='center'>{viicms:$vo.connectnum}</td>
				<td align='center'><?php if($vo['iscopyright'] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
				<td align='center'>{viicms:$vo.activitynum}</td>
				<td align='center'><?php if(($vo["status"]) == "1"): ?><font color="red">√</font><?php else: ?><font color="blue">×</font><?php endif; ?> 
				</td>
				<td align='center'>
					<a href="{viicms::U('User_group/edit/',array('id'=>$vo['id']))}">修改</a>
					| <?php if(($vo["username"]) == "admin"): ?><font color="#cccccc">删除</font><?php else: ?><a href="javascript:void(0)" onclick="return confirmurl('{viicms::U('User_group/del/',array('id'=>$vo['id']))}','确定删除该用户吗?')">删除</a><?php endif; ?>
				</td>
