...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="alist">
	  <tr>
	    ...
	    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td >{viicms:$role[$vo['role']]}</td>
				<td >{viicms:$vo.remark}</td>
				<td align='center'>{viicms:$vo.last_login_ip}</td>
				<td align='center'>{viicms:$vo.last_location}</td>
				...
				<td align='center'>
					<a href="{viicms::U('User/edit/',array('id'=>$vo['id']))}">修改</a>
					| <?php if(($vo["username"]) == "admin"): ?><font color="#cccccc">删除</font><?php else: ?><a href="javascript:void(0)" onclick="return confirmurl('{viicms::U('User/del/',array('id'=>$vo['id']))}','确定删除该用户吗?')">删除</a><?php endif; ?>
				</td>
